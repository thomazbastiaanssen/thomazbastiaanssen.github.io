...
p_stack <- ggplot(df_stack) + 
  ...
  annotate(
    geom = "RichText", x = c(1, 2), y = c(RSS1, RSS1) / 2, 
    size = 5, label.colour = c("mediumblue", "orange"), label.size = 1,
    label = 
      paste(c("RSS~1~ =", "RSS~0~ ="), round(c(RSS1, RSS0), 3)), 
           ) +
...
\begin{equation}
\tag{Our F-value}
F = \frac{35.922 - 16.192}{16.192}\times \frac{20 - 2}{2 - 1} = 21.93
\end{equation}
...
## Revisiting our F-distribution
Now, let's take a step back and consider the equation for the F-distribution 
from a different perspective. We'll start out by visualizing the RSS-bit of the 
equation. 

```{r stacked-revisit, fig.height=3, fig.width=10, echo=FALSE}
df_stack %>% 
  bind_rows(
    data.frame(
      id = "21", x = 1, y = RSSx, 
      x_lines = RSS0, y_lines = RSS0, 
      Hypothesis = factor("aH~1~", levels = c("H~0~", "aH~1~", "H~1~"))
      )
  ) %>% 
  mutate(Hypothesis = factor(Hypothesis, levels = c("H~0~", "aH~1~", "H~1~"))) %>% 
   ggplot() + 
  aes(x = x, y = y, colour = Hypothesis) + 
  ...
  geom_bar(stat = "identity", aes(fill = Hypothesis, linetype = Hypothesis), linewidth = 1, width = 3/4) +
  annotate(
    geom = "RichText", x = c(1, 2, 1), y = c(c(RSS1, RSS1) / 2, RSS1 + RSSx/2), 
    size = 5, label.colour = c("mediumblue", "orange", "#31a354"), label.size = 1,
    label = 
      paste(c("RSS~1~ =", "RSS~0~ =", "RSS~0~ - RSS~1~ = "), round(c(RSS1, RSS0, RSSx), 3)), 
           ) +
  scale_fill_manual(values = c("H~0~" = "gold", "H~1~" = "dodgerblue", "aH~1~" = "#addd8e")) + 
  scale_colour_manual(values = c("H~0~" = "orange", "H~1~" = "mediumblue", "aH~1~" = "#31a354")) +
  scale_linetype_manual(values = c("H~0~" = "solid", "H~1~" = "solid", "aH~1~" = "dashed")) +
...
In the equation, we break $RSS_0$ into two component chunks, proportional to the
size of $RSS_1$. Realize that each observation contributes twice in combined 
bottom bars.   